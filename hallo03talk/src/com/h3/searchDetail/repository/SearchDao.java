...
public class SearchDao {
	...
	public ArrayList<PlaceVo> pselectList(Connection conn){
		...
		String sql = "SELECT A.NO , A.NAME , A.CONTENT , A.ADDRESS , B.NAME FROM PLACE A LEFT OUTER JOIN PLACE_PHOTO B ON A.NO = B.PLACE_NO WHERE B.PHOTO_PROFILE = 'Y' AND A.TITLE LIKE '%검색' OR A.CONTENT LIKE '%검색%'";
	...
	public ArrayList<PlaceReviewVo> rselectList(Connection conn){
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		ArrayList<PlaceReviewVo> plist = new ArrayList<PlaceReviewVo>();
	...
	public ArrayList<CommVo> cselectList(Connection conn){
		...
		String sql = "SELECT A.NO, A.TITLE, A.CONTENT, A.WRITER, B.NAME FROM COMMUNITY A LEFT OUTER JOIN COMMUNITY_PHOTO B ON A.NO = B.COMMUNITY_NO WHERE B.STATUS = 'Y' AND A.TITLE LIKE '%검색' OR A.CONTENT LIKE '%검색%'";
