	...
	public List<PlaceVo> getList(Connection conn, String cityName, String insideName) {
		...
		try {
			if (!insideName.equals("모두")) {
				if (insideName.equals("시내")) {
					if (cityName.equals("제주시")) {
						sql = "SELECT P.NO NO,P.NAME NAME,P.CONTENT CONTENT,P.ADDRESS ADDRESS,P.BOSS_NO BOSS_NO,P.CATEGORY_NO CATEGORY_NO,P.ENROLL_DATE ENROLL_DATE,P.CNT CNT, Z.TRAVELER_NO TRAVELER_NO, PP.NAME PHOTONAME FROM PLACE P LEFT OUTER JOIN ZZIM Z ON P.NO = Z.PLACE_NO JOIN PLACE_PHOTO PP ON P.NO = PP.PLACE_NO WHERE P.STATUS='Y' AND PP.PHOTO_PROFILE='Y' AND P.ADDRESS LIKE '%'||?||'%' OR P.ADDRESS LIKE '%'||?||'%' OR P.ADDRESS LIKE '%'||?||'%' OR P.ADDRESS LIKE '%'||?||'%' OR P.ADDRESS LIKE '%'||?||'%' ORDER BY P.NO DESC";
						pstmt = conn.prepareStatement(sql);
